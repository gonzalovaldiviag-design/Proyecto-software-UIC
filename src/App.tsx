...
type Tab = 'inventario' | 'mantenimiento';
                      ...
                      filtered.map((eq) => (
                        <tr key={eq.id} className="group transition-colors hover:bg-slate-50/70">
                          <td className="px-5 py-4">
                            <div className="relative" ref={openMenuId === eq.id ? menuRef : undefined}>
                              <button
                                onClick={() => toggleMenu(eq.id)}
                                className="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 font-mono text-xs font-semibold text-blue-600 transition-colors hover:bg-blue-50"
                              >
                                {eq.codigo}
                                <ChevronDown className={`h-3.5 w-3.5 transition-transform ${openMenuId === eq.id ? 'rotate-180' : ''}`} />
                              </button>
                              {openMenuId === eq.id && (
                                <div className="absolute left-0 top-full z-20 mt-1 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-xl">
                                  <button
                                    onClick={() => { setOpenMenuId(null); openMantenimiento(eq); }}
                                    className="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 transition-colors hover:bg-amber-50 hover:text-amber-700"
                                  >
                                    <Wrench className="h-4 w-4 text-amber-500" />
                                    Ingresar Mantenimiento
                                  </button>
                                  <button
                                    onClick={() => { setOpenMenuId(null); openHojaVida(eq); }}
                                    className="flex w-full items-center gap-2.5 px-4 py-2.5 text-sm text-slate-700 transition-colors hover:bg-blue-50 hover:text-blue-600"
                                  >
                                  ...
                                  <button
                                    onClick={() => { setOpenMenuId(null); handleDelete(eq); }}
